...
export function DocumentsManager({ motherId }: { motherId: string }) {
  ...
  const handleDownload = async (fileName: string) => {
    try {
      // יצירת קישור זמני ומאובטח (תקף ל-60 שניות)
      const { data, error } = await supabase.storage
        .from("medical-docs")
        .createSignedUrl(`${motherId}/${fileName}`, 60);
      ...
      toast.error("שגיאה בפתיחת הקובץ: " + error.message);
    }
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {files.length === 0 && (
          <p className="text-muted-foreground text-sm">אין מסמכים בתיק</p>
        )}
